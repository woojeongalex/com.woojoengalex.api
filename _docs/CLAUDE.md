...
| 4 | `vault/woojeongai/db-rules.md` | ORM PK·`id`·refresh 규칙 |
...
| **Ordinal** (순서) | 자료 사이에 순서(서열)가 있음 | 1.매우낮음 2.낮음 3.보통 4.높음 5.매우높음 |

### Quantitative — 숫자로 셀 수 있는 데이터

| 척도 | 설명 | 예시 |
|------|------|------|
| **Interval** (간격) | 기준점 없이 일정한 측정 구간 — "몇 배" 비교 불가 | 온도, pH, 시간 구간 |
| **Ratio** (비율) | 절대적 원점(0)이 있어 "몇 배" 비교 가능 | 나이, 돈, 몸무게 |
