...
async def get_admin_stats(db: AsyncSession = Depends(get_db)):
    try:
        total_evals = (
            await db.scalar(select(func.count()).select_from(SingEvaluationEntity)) or 0
        )
        ...
        avg_score_raw = await db.scalar(
            select(
                func.round(
                    func.avg(
                        (
                            AiVocalAnalysisEntity.pitch_score
                            + AiVocalAnalysisEntity.rhythm_score
                        )
                        / 2.0
                    ),
                    1,
        ...
        passenger_count = (
            await db.scalar(select(func.count()).select_from(PersonOrm)) or 0
        )
...
async def get_service_stats(db: AsyncSession = Depends(get_db)):
    try:
        ...
        total_analyses = (
            await db.scalar(select(func.count()).select_from(AiVocalAnalysisEntity))
            or 0
        )
        total_speech = (
            await db.scalar(select(func.count()).select_from(SpeechEvaluationEntity))
            or 0
        )
        total_feedback = (
            await db.scalar(
                select(func.count()).select_from(SpeechFeedbackAnalysisEntity)
            )
            or 0
        )
        ...
        vocal_eval_rate = (
            min(round((total_analyses / total_evals) * 100), 100)
            if total_evals > 0
            else 0
        )
        passenger_data_rate = (
            min(round((survived_count / total_passengers) * 100), 100)
            if total_passengers > 0
            else 0
        )
        speech_eval_rate = (
            min(round((total_feedback / total_speech) * 100), 100)
            if total_speech > 0
            else 0
        )
...
async def get_admin_passengers(db: AsyncSession = Depends(get_db)):
    try:
        ...
        data = [
            {
                "id": r.id,
                "passengerId": int(r.passenger_id)
                if r.passenger_id and r.passenger_id.strip().isdigit()
                else r.id,
                "survived": int(r.survived)
                if r.survived and r.survived.strip().isdigit()
                else 0,
                "pclass": int(r.pclass)
                if r.pclass and r.pclass.strip().isdigit()
                else 0,
                "name": r.name or "",
                "gender": "male"
                if r.gender and r.gender.strip().lower() in ("male", "m")
                else "female",
                "age": float(r.age)
                if r.age and r.age.strip().replace(".", "", 1).isdigit()
                else None,
                "fare": float(r.fare)
                if r.fare and r.fare.strip().replace(".", "", 1).isdigit()
                else None,
