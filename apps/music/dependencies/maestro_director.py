...
from music.app.ports.output.vocal_maestro_analyzer_repository_port import MaestroRepositoryPort
from music.app.use_cases.vocal_maestro_analyzer_interactor import MaestroAnalyzerInteractor


def get_maestro_repository(db: AsyncSession = Depends(get_db)) -> MaestroRepositoryPort:
    return MaestroAnalyzerPgRepository(session=db)


def get_maestro_use_case(
    repository: MaestroRepositoryPort = Depends(get_maestro_repository),
