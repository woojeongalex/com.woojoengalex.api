...
@vocal_muse_recommender_router.get(
    "/api/music/muse/myself", response_model=MuseIntroduceResponse
)
async def muse_introduce_myself(
    muse: SuggestUseCase = Depends(get_suggest_use_case),
) -> MuseIntroduceResponse:
    return await muse.introduce_myself(
        MuseIntroduceSchema(id=3, name="보컬 뮤즈 (Vocal Muse)")
    )


@vocal_muse_recommender_router.post(
    "/api/music/vocal-recommendations", response_model=VocalRecommendationResponse
)
