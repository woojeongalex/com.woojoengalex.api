...
@instrument_franz_catalog_router.get(
    "/api/music/franz/myself", response_model=FranzIntroduceResponse
)
async def franz_introduce_myself(
    franz: InstrumentCatalogUseCase = Depends(get_instrument_catalog_use_case),
) -> FranzIntroduceResponse:
    return await franz.introduce_myself(
        FranzIntroduceSchema(id=4, name="악기 프란츠 (Franz)")
    )


@instrument_franz_catalog_router.get(
    "/api/music/instrument-catalog", response_model=InstrumentCatalogResponse
)
