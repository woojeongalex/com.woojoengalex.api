...
@speech_cicero_topic_router.get(
    "/api/music/cicero/myself", response_model=CiceroIntroduceResponse
)
async def cicero_introduce_myself(
    cicero: SpeechTopicUseCase = Depends(get_speech_topic_use_case),
) -> CiceroIntroduceResponse:
    return await cicero.introduce_myself(
        CiceroIntroduceSchema(id=7, name="스피치 키케로 (Cicero)")
    )


@speech_cicero_topic_router.get(
    "/api/music/speech-topics", response_model=SpeechTopicsResponse
)
