...
@speech_lumiere_video_router.get(
    "/api/music/lumiere/myself", response_model=LumiereIntroduceResponse
)
async def lumiere_introduce_myself(
    lumiere: VideoAnalysisUseCase = Depends(get_video_use_case),
) -> LumiereIntroduceResponse:
    return await lumiere.introduce_myself(
        LumiereIntroduceSchema(id=11, name="스피치 루미에르 (Lumiere)")
    )


@speech_lumiere_video_router.post(
    "/api/music/analyze-video", response_model=VideoVocalAnalysisResponse
)
