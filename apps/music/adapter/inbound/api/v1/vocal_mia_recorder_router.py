...
@vocal_mia_recorder_router.get(
    "/api/music/mia/myself", response_model=MiaIntroduceResponse
)
async def mia_introduce_myself(
    mia: EvaluationUseCase = Depends(get_evaluation_use_case),
) -> MiaIntroduceResponse:
    return await mia.introduce_myself(
        MiaIntroduceSchema(id=2, name="보컬 미아 (Vocal Mia)")
    )


@vocal_mia_recorder_router.post(
    "/api/music/sing-evaluation", response_model=VocalEvaluationResponse
)
