...
@vocal_bard_searcher_router.get(
    "/api/music/bard/myself", response_model=BardIntroduceResponse
)
async def bard_introduce_myself(
    bard: SearchUseCase = Depends(get_search_use_case),
) -> BardIntroduceResponse:
    return await bard.introduce_myself(
        BardIntroduceSchema(id=1, name="보컬 바드 (Vocal Bard)")
    )


@vocal_bard_searcher_router.get(
    "/api/songs/search", response_model=SongMrSearchResponse
)
