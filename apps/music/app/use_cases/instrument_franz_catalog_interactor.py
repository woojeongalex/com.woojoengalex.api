...
class FranzCatalogInteractor(InstrumentCatalogUseCase):
    ...
    async def introduce_myself(
        self, schema: FranzIntroduceSchema
    ) -> FranzIntroduceResponse:
        return await self.repository.introduce_franz(
            FranzIntroduceQuery(id=schema.id, name=schema.name)
        )
    ...
    def search(self, q: str = "") -> InstrumentCatalogResultDto:
        ...
        logger.info(
            "[MUSIC][franz][4/interactor] 카탈로그 검색 q=%r hits=%d", q, len(hits)
        )
