...
IUEM **음악·말하기** 도메인 ERD. 구현: `backend/apps/music` (SQLModel → Neon PostgreSQL).  
...
| `sing_evaluations` 점수·카탈로그·입력 미러 | **컬럼 제거**(ORM)·Neon DDL [`normalize_music_3nf.sql`](../../../backend/apps/music/migrations/normalize_music_3nf.sql) |
...
레거시 3NF: `backend/apps/music/migrations/normalize_music_3nf.sql` (경로 확정 시 Alembic으로 통합).
...
API: `GET /api/songs/search?q=` → `search_router` → 검색마다 일치 곡 **행 INSERT**
...
API: `POST /api/music/vocal-recommendations` · `GET /api/music/vocal-recommendations?singEvaluationId=` → `SuggestController`
...
API: `GET /api/music/instrument-catalog?q=` → `InstrumentController.list_catalog`
...
API: `POST /api/music/instrument-evaluation` → `InstrumentController` · 프론트 `/instrument`
...
API: `GET /api/music/speech-topics` → `SpeechController.list_topics`
...
모든 행에 `created_at`/`updated_at`. API: `POST /api/music/speech-evaluation` → `speech_router` · 프론트 `/speech`

## 비-DB 처리 (다이어그램 외)

| 구성 | 역할 | 비고 |
|------|------|------|
| `POST /api/music/analyze-video` | multipart 영상 → WAV → librosa 분석 | `VideoAnalysisController` / `video_analysis_service.py` |
...
| 레이어 | MR 검색 | 보컬 | 악기 | 스피치 | 추천 | 영상 |
|--------|---------|------|------|--------|------|------|
| Router (`adapter/inbound/api/v1`) | `search_router` | `evaluation_router` | `instrument_router` | `speech_router` | `suggest_router` | `video_router` |
| Use Case (`app/use_cases`) | `list_service` | `evaluation_service` | `instrument_service` | `speech_service` | `suggest_service` | `video_analysis_service` |
| PG (`adapter/outbound/pg`) | `list_pg_repository` | `evaluation_pg_repository` | `instrument_pg_repository` 등 | 동일 | `suggest_pg_repository` | — |
| HTTP Schema (`adapter/inbound/api/schemas`) | `list_schema` | `evaluation_schemas` | `instrument_schemas` | `speech_schemas` | `suggest_schema` | `video_analysis_schema` |
