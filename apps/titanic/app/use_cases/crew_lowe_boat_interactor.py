...
class LoweBoatInteractor(LoweBoatUseCase):
    ...
    def feature_engineering(
        ...
        train["gender"] = (
            train["gender"].map({"male": 0, "female": 1}).fillna(0).astype(int)
        )
        if "gender" in test.columns:
            test["gender"] = (
                test["gender"].map({"male": 0, "female": 1}).fillna(0).astype(int)
            )
        ...
        for df in [train, test]:
            if "age" in df.columns:
                ...
                if "Title" in df.columns:
                    ...
                    df.loc[mask, "AgeGroup"] = df.loc[mask, "Title"].map(
                        age_title_mapping
                    )
                df["AgeGroup"] = (
                    pd.to_numeric(df["AgeGroup"].map(age_mapping), errors="coerce")
                    .fillna(0)
                    .astype(int)
                )
        ...
        for df in [train, test]:
            if "embarked" in df.columns:
                df["embarked"] = (
                    pd.to_numeric(
                        df["embarked"].fillna("S").map({"S": 1, "C": 2, "Q": 3}),
                        errors="coerce",
                    )
                    .fillna(1)
                    .astype(int)
        ...
        for df in [train, test]:
            if "fare" in df.columns:
                ...
                df["FareBand"] = (
                    pd.qcut(
                        df["fare"], 4, labels=list(range(1, _n + 1)), duplicates="drop"
                    )
                    .astype(float)
                    .fillna(1)
                    .astype(int)
